...
/*
 * UART0 interrupt source UART0 的中断优先级
 * see https://github.com/qemu/qemu/blob/master/include/hw/riscv/virt.h
 * enum {
 *     UART0_IRQ = 10,
 *     ......
 ...
 *     (VIRT_PLIC_CONTEXT_BASE + (__num_context) * VIRT_PLIC_CONTEXT_STRIDE)
    VIRT_PLIC_HART_CONFIG：表示对于每个Hart（硬件线程），其配置信息的字符串。
    VIRT_PLIC_NUM_SOURCES：表示PLIC支持的中断源数量，这里是127个。
    VIRT_PLIC_NUM_PRIORITIES：表示PLIC支持的中断优先级数量，这里是7个。
    VIRT_PLIC_PRIORITY_BASE：表示PLIC中优先级寄存器的基地址。
    VIRT_PLIC_PENDING_BASE：表示PLIC中挂起寄存器的基地址。
    VIRT_PLIC_ENABLE_BASE：表示PLIC中使能寄存器的基地址。
    VIRT_PLIC_ENABLE_STRIDE：表示PLIC中使能寄存器的跨度（每个使能寄存器的地址之间的间隔）。
    VIRT_PLIC_CONTEXT_BASE：表示PLIC中上下文寄存器的基地址。
    VIRT_PLIC_CONTEXT_STRIDE：表示PLIC中上下文寄存器的跨度（每个上下文寄存器的地址之间的间隔）。
    VIRT_PLIC_SIZE(__num_context)：根据给定的上下文数量计算PLIC的总大小。
 */
