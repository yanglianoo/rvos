 ...
 * 这个宏定义是用于返回UART控制寄存器的地址，
 * 其中UART0是UART控制寄存器的基地址，reg是相对于基地址的偏移量。
 * 宏定义中使用了类型转换运算符，将返回的地址转换为一个指向8位无符号整数的指针，
 * 并加上了volatile关键字，表示该地址指向的内存中的值可能会被程序以外的因素修改，需要避免编译器对该值进行优化。
 */
